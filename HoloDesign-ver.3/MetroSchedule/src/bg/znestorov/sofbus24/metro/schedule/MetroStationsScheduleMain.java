...
public class MetroStationsScheduleMain {
	...
	public static MetroStation retrieveStationsSchedule(Logger logger,
		...
		if ("".equals(weekdayHtmlResponse) || "".equals(holidayHtmlResponse)) {
			logger.warning(
					"Problem with retrieving information about station name = "
							+ station.getValue() + " and number = "
							+ station.getKey());
			return null;
		}
		...
		if (ms == null) {
			logger.warning(
					"Problem with parsing the schedule information from SGKT...");
			return null;
		}
	...
	public static void saveStationsScheduleToAFile(Logger logger,
		...
		if (metroDirectionsStations.size() == 4) {
				...
				for (int j = 0; j < count; j++) {
					...
					if (ms1 != null && ms2 != null) {
						...
						if (ms1.getNumber().equals(ms2.getNumber())) {
							...
							ms1.setDirection(
									Utils.formatDirection(ms1.getDirection()));
			...
			logger.warning(
					"Problem with merging the schedule information for the stations...");
		}
	}
}
